#include "Board.hpp"
#include <iostream>
#include <string>

Board::Board() {
    // Initialize the board with the starting position
    std::string initialBoard[ROWS][COLS] = {
        {"   ", " a ", " b ", " c ", " d ", " e ", " f ", " g ", " h "},
        {" 1 ", "[R]", "[K]", "[B]", "[Q]", "[K]", "[B]", "[K]", "[R]"},
        {" 2 ", "[P]", "[P]", "[P]", "[P]", "[P]", "[P]", "[P]", "[P]"},
        {" 3 ", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]"},
        {" 4 ", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]"},
        {" 5 ", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]"},
        {" 6 ", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]", "[ ]"},
        {" 7 ", "[p]", "[p]", "[p]", "[p]", "[p]", "[p]", "[p]", "[p]"},
        {" 8 ", "[r]", "[k]", "[b]", "[k]", "[q]", "[b]", "[k]", "[r]"}
    };

    // Copy the initial board to the member grid
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLS; j++) {
            grid[i][j] = initialBoard[i][j];
        }
    }
}

void Board::display() const {
    for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLS - 1; j++) {  // COLS - 1 to avoid printing extra spaces
            std::cout << grid[i][j];
        }
        std::cout << std::endl;
    }
}

void Board::movePiece(int s_col, int s_row, int m_col, int m_row) {
    std::string temp_space = grid[m_row][m_col];
    grid[m_row][m_col] = grid[s_row][s_col];
    grid[s_row][s_col] = temp_space;
}

void Board::selectPiece(int col, int row) {
    std::cout << col << " " << row << std::endl;
    
    std::string move;
    std::getline(std::cin, move);

    int m_col = move[0] - 'a' + 1;
    int m_row = move[1] - '1' + 1;

    // Remove the grid parameter - it's a member function so it has access to grid
    movePiece(col, row, m_col, m_row);
}

void Board::receiveInputs() {
    std::string select;
    std::getline(std::cin, select);

    int col = select[0] - 'a' + 1;
    int row = select[1] - '1' + 1;

    selectPiece(col, row);
}